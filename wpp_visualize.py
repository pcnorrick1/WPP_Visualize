...
from plotnine import aes, annotate, geom_line, geom_point, ggplot, ggtitle, guides
...
def main():
    ...
    wpp_low_2022 = pd.read_csv(data_processed / "WPP_2022_Low_clean.csv")

    wpp_median_2024 = pd.read_csv(data_processed / "WPP_2024_Median_clean.csv")
    wpp_low_2024 = pd.read_csv(data_processed / "WPP_2024_Low_clean.csv")

    vital_statistics = pd.read_csv(
        data_processed / "vital_statistics.csv", encoding="unicode_escape"
    )
    ...
    variable_list = wpp_median_2022.columns[
        10:-2
    ]  # Only want numerical columns as options for display
    ...
    )  # Dropdown list of variables

    # Plotting Based on User Selections
    location_code = wpp_median_2022.loc[
        wpp_median_2022["Country"] == selected_country, "Location code"
    ]
    location_code = location_code.iloc[0]
    wpp_median_2022 = wpp_median_2022.loc[
        wpp_median_2022["Location code"] == location_code
    ]
    wpp_low_2022 = wpp_low_2022.loc[wpp_low_2022["Location code"] == location_code]
    wpp_median_2024 = wpp_median_2024.loc[
        wpp_median_2024["Location code"] == location_code
    ]
    ...
    p = ggplot(
        data=full_df, mapping=aes(x="Year", y=f"{selected_variable}")
    ) + geom_line(aes(color="Variant", linetype="Projection"))
    ...
    if vital_stats_selected:
        ...
        if not vital_stats_row.empty:
            ...
            if (not vital_stat_row_one.empty) and (
                float(vital_stat_row_one[selected_variable]) > 0
            ):
                p = p + geom_point(
                    aes(
                        x=vital_stat_row_one["Year"],
                        y=float(vital_stat_row_one[f"{selected_variable}"]),
                    ),
                    color="black",
                )
                if (
                    (year_limits[1] - year_limits[0] <= 30)
                    and (year_limits[0] <= 2023)
                    and (year_limits[1] >= 2024)
                ):
                    p = p + annotate(
                        "text",
                        x=1.5 + vital_stat_row_one["Year"],
                        y=float(vital_stat_row_one[f"{selected_variable}"]),
                        label="2023",
                    )
            if (not vital_stat_row_two.empty) and (
                float(vital_stat_row_two[selected_variable]) > 0
            ):
                p = p + geom_point(
                    aes(
                        x=vital_stat_row_two["Year"],
                        y=float(vital_stat_row_two[f"{selected_variable}"]),
                    ),
                    color="black",
                )
                if (
    ...
    p = (
        p
        + ggtitle(f"{selected_country}, {selected_variable}")
